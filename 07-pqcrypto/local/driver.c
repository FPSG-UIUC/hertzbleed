...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < outer * num_selectors; i++) {
		...
		char command[256];
		sprintf(command, "../PQCrypto-SIDH/sike751/test_SIKE_LOCAL %d %d %d %d 1000000000 0", arg.flipkey, arg.keyindex, arg.bitindex, arg.number_thread);
		printf("Running: %s\n", command);

		// Start LOCAL
		pthread_create(&thread1, NULL, (void *)&victim, (void *)command);

		// Wait 20 seconds before starting the monitor
		sleep(20);
		...
		// Stop stress
		system("pkill -f test_SIKE_LOCAL"); 
